...
class CameraManager:
    def __init__(self, config: Dict[str, Any], logger: Any) -> None:
        ...
        self.source = vision_config.get("source", "camera").lower()
        ...
        self.video_extensions = tuple(
            vision_config.get("video_extensions", [".mp4", ".avi", ".mov", ".mkv"])
        )
    ...
    def start_camera(self) -> cv2.VideoCapture:
        if self.source == "video_folder":
            ...
            self.current_video_index = 0
            cap = self._open_video_file(self.video_files[self.current_video_index])
            source_name = str(self.video_files[self.current_video_index])
    ...
    def has_next_video(self) -> bool:
        return self.source == "video_folder" and self.current_video_index + 1 < len(self.video_files)
    ...
    def is_video_source(self) -> bool:
        return self.source in {"video", "video_folder"}

    def is_video_folder_source(self) -> bool:
        return self.source == "video_folder"